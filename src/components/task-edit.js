...
const isAllowableDescriprionLength = (description) => {
...
const createTaskEditTemplate = (task, options = {}) => {
  ...
  const {currentDescription, isDateShowing, isRepeatingTask, activeRepeatingDays, externalData} = options;

  const description = window.he.encode(currentDescription);

  const isExpired = dueDate instanceof Date && isOverdueDate(dueDate, new Date());
  const isBlockSaveButton = (isDateShowing && isRepeatingTask) || (isRepeatingTask && !isRepeating(activeRepeatingDays)) || !isAllowableDescriprionLength(description);
  ...
  _subscribeOnEvents() {
    ...
    element.querySelector(`.card__text`)
      .addEventListener(`input`, (evt) => {
        ...
        saveButton.disabled = !isAllowableDescriprionLength(this._currentDescription);
      });
